...
###### Adding Files ######
wget -O /content/Leech-Pro/app/tobrot/aria2/dht.dat https://github.com/P3TERX/aria2.conf/raw/master/dht.dat
wget -O /content/Leech-Pro/app/tobrot/aria2/dht6.dat https://github.com/P3TERX/aria2.conf/raw/master/dht6.dat
...
if [[ -n $RCLONE_CONFIG_URL ]]; then
  ...
  wget -q $RCLONE_CONFIG_URL -O /content/Leech-Pro/app/rclone.conf
fi

if [[ -n $CONFIG_ENV_URL ]]; then
  echo " Found config.env File 📁📁 "
	wget -q $CONFIG_ENV_URL -O /content/Leech-Pro/app/config.env
fi
